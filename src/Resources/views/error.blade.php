...
<div class="trace-error-page">
  ...
  <div class="bx">
    ...
    <div class="db">
      ...
      <div class="db-b">@foreach($list as $it)<div class="di"><div class="dl">{{$it['label']??''}}</div><div class="dv">@if(($it['type']??'')==='code_html')<pre>{!!$it['value']??''!!}</pre>@elseif(($it['type']??'')==='code')<pre><code>{{$it['value']??''}}</code></pre>@elseif(($it['type']??'')==='debug_file')@php $ed=$it['editor']??config('trace.editor','phpstorm');$f=$it['file']??'';$ln=$it['line']??1;@endphp<a href="{{$ed}}://open?file={{urlencode($f)}}&line={{$ln}}" style="color:#7f9cf5;text-decoration:none;padding:5px 12px;background:rgba(102,126,234,.08);border-radius:8px;font-size:13px;display:inline-flex;align-items:center;gap:5px;transition:all .3s" onmouseover="this.style.background='rgba(102,126,234,.18)';this.style.color='#a78bfa'" onmouseout="this.style.background='rgba(102,126,234,.08)';this.style.color='#7f9cf5'"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>{{$it['value']??$f}}</a>@else{{is_string($it['value']??'')?$it['value']:json_encode($it['value'],JSON_UNESCAPED_UNICODE)}}@endif</div></div>@endforeach</div>
    </div>
    @endif

    @if($isDb && isset($exception) && is_object($exception))
    @php $exEditor = config('trace.editor', 'phpstorm'); $exFile = $exception->getFile(); $exLine = $exception->getLine(); @endphp
    <div class="db" style="margin-top:16px">
      ...
      <div class="db-b">
        ...
        <div class="di"><div class="dl">文件</div><div class="dv"><a href="{{$exEditor}}://open?file={{urlencode($exFile)}}&line={{$exLine}}" style="color:#7f9cf5;text-decoration:none;padding:5px 12px;background:rgba(102,126,234,.08);border-radius:8px;font-size:13px;display:inline-flex;align-items:center;gap:5px;transition:all .3s" onmouseover="this.style.background='rgba(102,126,234,.18)';this.style.color='#a78bfa'" onmouseout="this.style.background='rgba(102,126,234,.08)';this.style.color='#7f9cf5'"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>{{str_replace(base_path(), '', $exFile)}}:{{$exLine}}</a></div></div>

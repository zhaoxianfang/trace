...
function _t_mem(): string {
  ...
  $lb=match($c){'G'=>$v*1073741824,'M'=>$v*1048576,'K'=>$v*1024,default=>$v};
  return _t_bytes($u).'/'. _t_bytes($lb).' ('.($lb>0?round($u/$lb*100,1):0).'%)';
  }catch(\Throwable){return '?';}
}
function _t_dbg(): bool { $d=$_ENV['APP_DEBUG']??$_SERVER['APP_DEBUG']??false;return is_string($d)?in_array(strtolower($d),['true','1','yes','on'],true):(bool)$d; }
...
set_error_handler(function(int $s, string $m, string $f, int $l) {
  ...
  if($c&&!$_trace_child){_t_render(500,'RESOURCE_EXHAUSTED',"资源耗尽: {$m}","1) 限制资源使用数量\n2) 确保资源正确释放\n3) 检查系统 ulimit",$f,$l);exit(1);}
  return $c;
});
...
/* ═══ Shutdown + 内存预留 ═══ */
$_trace_mem = str_repeat('x', 192 * 1024); // 192KB 预留（减少 25%）
register_shutdown_function(function() use (&$_trace_mem) {
  global $_trace_last, $_trace_crit, $_trace_id;
  $_trace_mem = ''; // 释放
  $e = error_get_last();
  $skip = headers_sent();
  if(!$skip&&$_trace_crit&&$_trace_last!==null){$d=$_trace_last;_t_render(500,'RESOURCE_EXHAUSTED',$d['message']??'资源耗尽','请优化资源使用',$d['file']??'',$d['line']??0);exit(1);}
  if(!$skip&&$_trace_last!==null&&!empty($_trace_last['type'])){$d=$_trace_last;_t_render(500,$d['type'],$d['message']??'错误','请检查代码',$d['file']??'',$d['line']??0);exit(1);}
  if($e===null||!_t_fatal($e['type']))return;
  _t_render(match($e['type']){E_ERROR=>507,default=>500},_t_names($e['type']),$e['message']??'致命错误','请检查错误日志',$e['file']??'',$e['line']??0);
  exit(1);
});
